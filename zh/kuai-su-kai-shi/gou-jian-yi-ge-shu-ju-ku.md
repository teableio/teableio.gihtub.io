...
当 Base 积累了足够多的数据之后，你可能需要进行数据洞察和分析，比如使用 Metabase 这种开源数据分析工具来制作数据报表和仪表盘。大部分数据分析工具都支持 Postgres 连接来获取数据，你可以直接在 Teable 中创建一个[ Postgres 数据库连接](../kai-fa-zhe/shu-ju-ku-lian-jie.md)，这样就可以让第三方工具读取到你 Base 中的数据。
